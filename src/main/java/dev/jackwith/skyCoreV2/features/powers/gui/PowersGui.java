...
import dev.jackwith.skyCoreV2.database.PowersCollection;
import dev.jackwith.skyCoreV2.database.data.PlayerPowerData;
import dev.jackwith.skyCoreV2.database.data.PowerData;
...
public class PowersGui {
    ...
    private void renderCurrentPower(Map<Integer, ItemStack> map) {
        try {
            PowersCollection db = SkyCore.getPowersCollection();
            PlayerPowerData data = db.getPlayerPower(player.getUniqueId());
            String power = data.getPower();

            ItemStack head = createHead(player,
                    Collections.singletonList(MM.deserialize(Optional.ofNullable(SkyCore.getPowerManager().getCachedPower(power))
    ...
    private void renderPowers(Map<Integer, ItemStack> map) {
        try {
            ...
            for (PowerData p : SkyCore.getPowerManager().getCachedPowers()) {
