...
import dev.jackwith.skyCoreV2.database.SalesCollection;
...
public class SellBoostCommand extends BaseCommand {
    ...
    private final SalesCollection salesCollection;

    public SellBoostCommand() {
        this.salesCollection = SkyCore.getSalesCollection();
    }

    @Default
    public void DefaultCommand(Player player) {
        double boost = salesCollection.getPlayerSellBoost(player.getUniqueId());
    ...
    public void Ccheck(CommandSender sender, String targetName) {
        ...
        double boost = salesCollection.getPlayerSellBoost(target.getUniqueId());
    ...
    public void setboost(CommandSender sender, String targetName, double value) {
        ...
        salesCollection.setPlayerSellBoost(target.getUniqueId(), value);
    ...
    public void addboost(CommandSender sender, String targetName, double value) {
        ...
        double newBoost = salesCollection.getPlayerSellBoost(uuid) + value;
        salesCollection.setPlayerSellBoost(uuid, newBoost);
    ...
    public void removeboost(CommandSender sender, String targetName, double value) {
        ...
        UUID uuid = target.getUniqueId();
        double newBoost = Math.max(1.0, salesCollection.getPlayerSellBoost(uuid) - value);
        salesCollection.setPlayerSellBoost(uuid, newBoost);
    ...
    public void resetboost(CommandSender sender, String targetName) {
        ...
        salesCollection.setPlayerSellBoost(target.getUniqueId(), 1.0);
